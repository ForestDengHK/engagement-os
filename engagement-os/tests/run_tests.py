...
Sized from the buyer's own volumetrics: 18 loads, 3 environments. Reconciliation held.
...
"""
    ...
    wb_checks += [
        ...
        # …and it must still live IN the workbook — now as rows on one sheet rather than as a
        # tab per topic, which is what made a third of the workbook read as filler.
        ("the judgement travels in the workbook, on one sheet",
         bw.JUDGEMENT_SHEET in nwb.sheetnames
         and {r[0].value for r in nwb[bw.JUDGEMENT_SHEET].iter_rows(min_row=2)}
         >= {name for name, _ in bw.NARRATIVE_SHEETS}),
        ("judgement text written in the markdown reaches the sheet",
         any("buyer's own volumetrics" in str(c.value)
             for r in nwb[bw.JUDGEMENT_SHEET].iter_rows() for c in r if c.value)),
        ("an unwritten topic says so in its own row",
         any(str(c.value).strip() == "<not yet written>"
             for r in nwb[bw.JUDGEMENT_SHEET].iter_rows() for c in r if c.value)),
    ...
    # A workbook is judged by its tab bar. Eight of twenty-one tabs holding four words each is
    # filler, and an empty tab named `BasisOfEstimate` makes a missing basis of estimate look
    # present — so the judgement is one sheet of rows, and no sheet exists with nothing to say.
    filler = []
    for w in sched_wb.worksheets:
        rows = [r for r in w.iter_rows()
                if any(c.value is not None and str(c.value).strip() for c in r)]
        words = sum(len(str(c.value).split()) for r in rows for c in r if c.value)
        if words < 25 and w.title not in ("Gantt",):     # Gantt is formulas, not prose
            filler.append(w.title)
    jud = sched_wb["Judgement"] if "Judgement" in sched_wb.sheetnames else None
    jud_topics = [r[0].value for r in jud.iter_rows(min_row=2)] if jud else []
    wb_checks += [
        ("no worksheet is a near-empty placeholder", not filler),
        ("judgement is one sheet, not one tab per topic",
         jud is not None and all(n not in sched_wb.sheetnames for n, _p in bw.NARRATIVE_SHEETS)),
        ("every judgement topic has a row",
         all(n in jud_topics for n, _p in bw.NARRATIVE_SHEETS)),
        ("unwritten judgement is counted out loud",
         any("topics unwritten" in str(c.value) for r in jud.iter_rows() for c in r if c.value)),
        ("tabs are ordered the way the model computes",
         [t for t in sched_wb.sheetnames if t in bw.SHEET_ORDER]
         == [t for t in bw.SHEET_ORDER if t in sched_wb.sheetnames]),
    ]
